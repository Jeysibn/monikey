...
export async function deliverNotificationOutbox(prisma: PrismaClient, provider: EmailProvider, limit = 25): Promise<number> {
  ...
  for (let index = 0; index < limit; index += 1) {
    ...
    try {
      ...
      const payload = candidate.payload as { merchant?: string; amountMinor?: number; dueDate?: string }
      await provider.send({ to: user.email, subject: 'Monikey bill reminder', text: `${payload.merchant ?? 'A recurring bill'} of ${((payload.amountMinor ?? 0) / 100).toFixed(2)} is due on ${payload.dueDate ?? 'an upcoming date'}.` })
